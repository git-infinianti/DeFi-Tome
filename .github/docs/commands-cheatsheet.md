# Evrmore Command Cheatsheet
Legend:
- Required args: plain values
- Optional args: `( ... )`
- Arrays/objects: `[ ... ]` / `{ ... }`

Validation Snapshot:
- Source: live RPC `help` responses from local node.
- Last full sweep: 2026-08-03.
- Commands listed: 171.
- Commands recognized by node: 171/171.

Refresh Procedure:
- Run `help "command"` against your node for any entry you modify.
- For a full recheck, script against RPC `help` for every command listed in this file and flag unknown commands.
- Recommended full sweep command: `python3 scripts/verify_rpc_cheatsheet.py --strict`.
- CI guardrail: `.github/workflows/verify-rpc-cheatsheet.yml` runs strict verification on pull requests.
...
Verified against live `help` RPC output on 2026-08-03.
...
Notes:
- `signrawtransactionwithkey` is not available on this node (`help: unknown command`).
- Use `signrawtransaction` with `privkeys` (3rd optional argument) for explicit key signing.
- `testmempoolaccept` requires an array of raw tx hex strings, even for a single tx.
